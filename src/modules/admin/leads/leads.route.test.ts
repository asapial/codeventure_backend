...
const calls = vi.hoisted(() => ({ list: vi.fn((_q, r) => r.json({ data: [] })), create: vi.fn((_q, r) => r.status(201).json({ data: {} })), update: vi.fn((_q, r) => r.json({ data: {} })), job: vi.fn((_q, r) => r.status(202).json({ jobRef: "j" })) }));
vi.mock("../admin.policy", () => ({ requireAdmin: (_q: unknown, _r: unknown, n: () => void) => n(), requirePermission: () => (_q: unknown, _r: unknown, n: () => void) => n() }));
vi.mock("../admin.handlers", () => ({ createAdminHandlers: () => ({ list: calls.list, create: () => calls.create, update: () => calls.update, job: () => calls.job }) }));
